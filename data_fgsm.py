...
'''
利用在1w张原始cifar10上训练好的Resnet模型来生成cxifar10的对抗样本
...
def test(trainloader, model, epsilon):
    ...
    for (inputs, soft_labels) in trainloader:
        ...
        #
        # perturbed_inputs = projected_gradient_descent(model, inputs, 1., 0.01, 40, np.inf)
        # #
        # outputs = model(perturbed_inputs)
        # acc = accuracy(outputs, targets)
        # accs_adv.update(acc[0].item(), inputs.size(0))
...
if __name__ == '__main__':
    ...
    # 保存生成的对抗样本用于下一步训练
    np.save('data_adv_2w.npy', images_glob)
    np.save('label_adv_2w.npy', labels_glob)
